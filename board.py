...
class GoBoard(object):
    ...
    def is_move_suicide(self, color, pos):
        ...
        #print("Move Suicide Time ", end - start)
    ...
    #def is_move_legal(self, color, pos):
        #'''Check if a proposed moved is legal.'''
        #return (not self.is_move_on_board(pos)) and \
            #(not self.is_move_suicide(color, pos))
    ...
    def get_all_legal_moves(self, color):
        ...
        for x in range(19):
            for y in range(19):
                ...
                start = time.time()
                if self.is_move_legal(color, (x, y)):
                    moves.append((x, y))
                end = time.time()
                #print("------ Is Move Legal time elapsed: ", end - start)
    ...
    def check_for_open_moves(self):
        for x in range(19):
            for y in range(19):
                if self.is_move_legal('b', (x, y)):
                    return True
                elif self.is_move_legal('w', (x, y)):
                    return True
                else:
                    return False

    def is_game_over(self):
        return not self.check_for_open_moves()
...
if __name__ == "__main__":
    goBoard = GoBoard()
    for i in range(19):
        goBoard.apply_move("b", goBoard.get_random_move("b"))
        goBoard.apply_move("w", goBoard.get_random_move("w"))
    print(goBoard)
